...
public class ftRenderLightmap : EditorWindow//ScriptableWizard
{
    ...
    void OnGUI()
    {
        ...
        GUI.Label(aboutRect, new GUIContent("<color=#5073c9ff><b>Bakery - GPU Lightmapper by Mr F</b></color>", "Version 1.9. Click to go to Bakery Wiki"), linkStyle);
            ...
            if (showCheckerSettings)
            {
                ...
                if (showChecker)
                {
                    ...
                    if (GUI.Button(new Rect(10+220/2, ay, 220/2, 20), "Atlas preview"))
                    {
                        if (ftAtlasPreview.instance == null)
                        {
                            var pwin = GetWindow<ftAtlasPreview>();
                        }
                        else
                        {
    ...
    static int ComparePointLights(BakeryPointLight a, BakeryPointLight b)
    {
        ...
        float compA = channelA * 10000 + ((a.bakeToIndirect && !a.shadowmask) ? 1000 : 0) + a.indirectIntensity;

        int channelB = GetShadowmaskChannel(b);
        float compB = channelB * 10000 + ((b.bakeToIndirect && !b.shadowmask) ? 1000 : 0) + b.indirectIntensity;
